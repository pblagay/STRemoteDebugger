...
static Diss68kOptions options = (Diss68kOptions)(doptOpcodesSmall | doptRegisterSmall | doptStackSP);
...
static int Disass68kLoadTextFile(const char *filename, char **filebuf)
{
	...
	char	*fbuf = (char*)malloc(fileLength);
	if(!fbuf) 
		return 0;
...
static void	Disass68kLoadStructInfo(const char *filename)
{
	...
	for(int i=0; i<lineCount; line = nextLine, ++i)
	{
		...
		if(line[0] == '{')
		{
			...
			se->count = 0;
			se->elements = (disStructElement*)malloc(sizeof(disStructElement) * lineCount);	// lineCount is way too much, but safe
		} else if(line[0] == '}') {
			if(se)
			{
				se->size = 0;
...
static void	Disass68kLoadSymbols(const char *filename)
{
	...
	int		lineCount = Disass68kLoadTextFile(filename, &fbuf);
	if(!lineCount) { if(fbuf) free(fbuf); return; }
	disSymbolEntries = (disSymbolEntry*)realloc(disSymbolEntries, sizeof(disSymbolEntry) * (disSymbolCounts + lineCount));
	...
	for(int i=0; i<lineCount; line = nextLine, ++i)
	{
		...
		disSymbolEntries[disSymbolCounts].type = (Disass68kDataType)type;
...
static Disass68kDataType Disass68kType(long addr, char *addressLabel, char *commentBuffer, int *count)
{
	...
	for(int i=0; i<disSymbolCounts; ++i)
	{
		...
		*count = 1;
		...
		return (Disass68kDataType)dse->size;
	}
	return dtNone;
}
